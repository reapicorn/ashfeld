...
function getPool() {
  if (!_pool) {
    _pool = new Pool({
      ...
      database: process.env.PGDATABASE || 'darkhorn_mq',
...
async function getUserById(id) {
  const { rows } = await getPool().query(`SELECT * FROM users WHERE id::text = $1 OR username = $1 LIMIT 1`, [id]);
...
async function updateUser(id, updates) {
  ...
  const values = [];
  let i = 1;
  const map = { email:'email', firstName:'first_name', lastName:'last_name', department:'department', title:'title', status:'status' };
  for (const [k, col] of Object.entries(map)) {
    if (updates[k] !== undefined && updates[k] !== null && updates[k] !== '') {
      fields.push(`${col} = $${i++}`); values.push(updates[k]);
    }
  }
...
async function assignGroups(userId, groupIds) {
  ...
  for (const gid of groupIds) {
    await pool.query(`INSERT INTO user_groups (user_id,group_id) VALUES ($1,$2) ON CONFLICT DO NOTHING`, [user.id, gid]);
  }
...
module.exports = {
  ...
  createUser, updateUser, deleteUser, setUserStatus, getUserPassword, setUserPassword,
