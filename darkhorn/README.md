...
Every company in Darkhorn handles the same three things — who works here, what they can access, and whether they're still active. The paperwork is identical. The process is not.
...
curl -s -u grimreaper:'Wh1sp3r0fD4rk!' \
  'http://localhost:3000/api/users?count=3'
...
```bash
sftp -P 2222 spectral@localhost
...
```bash
# WSDL
curl -s 'http://localhost:3002/soap?wsdl' | head -10
...
# LookupUser
curl -s -X POST http://localhost:3002/soap \
...
docker exec darkhorn-rabbitmq rabbitmqadmin -H localhost -u darkhorn -p 'Wr41thPuls3!' \
...
# Search users
curl -s -u grimreaper:'Wh1sp3r0fD4rk!' \
  'http://localhost:3000/api/users?count=3' | jq .

# Lookup user
curl -s -u grimreaper:'Wh1sp3r0fD4rk!' \
  'http://localhost:3000/api/users/test.user' | jq .
...
ID=$(curl -s -u grimreaper:'Wh1sp3r0fD4rk!' \
  'http://localhost:3000/api/users/test.user' | grep -o '"id":"[^"]*"' | cut -d'"' -f4)
...
# Suspend / restore
curl -s -X POST http://localhost:3000/api/users/$ID/suspend  -u grimreaper:'Wh1sp3r0fD4rk!'
curl -s -X POST http://localhost:3000/api/users/$ID/restore  -u grimreaper:'Wh1sp3r0fD4rk!'

# Change password
curl -s -X POST http://localhost:3000/api/users/$ID/change-password \
  -u grimreaper:'Wh1sp3r0fD4rk!' \
  -H 'Content-Type: application/json' \
  -d '{"currentPassword": "D4rkh0rn!", "newPassword": "N3wD4rkh0rn!"}'

# Reset password
curl -s -X POST http://localhost:3000/api/users/$ID/reset-password \
  -u grimreaper:'Wh1sp3r0fD4rk!' \
  -H 'Content-Type: application/json' \
...
# Get user groups
curl -s -u grimreaper:'Wh1sp3r0fD4rk!' \
  "http://localhost:3000/api/users/$ID/groups" | jq .

# Assign groups
curl -s -X POST http://localhost:3000/api/users/$ID/groups \
  -u grimreaper:'Wh1sp3r0fD4rk!' \
  -H 'Content-Type: application/json' \
  -d '{"groupNames": ["admins", "developers"]}'

# Remove groups
curl -s -X DELETE http://localhost:3000/api/users/$ID/groups \
  -u grimreaper:'Wh1sp3r0fD4rk!' \
  -H 'Content-Type: application/json' \
  -d '{"groupNames": ["developers"]}'

# Delete user
curl -s -X DELETE http://localhost:3000/api/users/$ID -u grimreaper:'Wh1sp3r0fD4rk!'
```
...
| Parameter | Value |
|---|---|
| JDBC URL | `jdbc:postgresql://localhost:5432/darkhorn_jdbc` |
...
| Parameter | Value |
|---|---|
| AMQP URL | `amqp://darkhorn:Wr41thPuls3!@localhost:5672` |
