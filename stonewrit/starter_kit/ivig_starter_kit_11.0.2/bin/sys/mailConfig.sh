...
if [[ "$1" = "--help" ]]; then
        ...
        echo "   It will Configure Mailing for ISVGIM Pod."
        echo "Example usage:"
        echo "   $ ./mailConfig.sh"
        echo ""
        exit 0
...
if [[ -f ../data/enRoleMail.properties ]]; then
	echo "Mail already configured!"
	exit
fi

IM_FILES="enRoleMail.properties enRole.properties"
...
$kubectl -n "$NS" exec "$POD" -- /bin/bash -c "/work/mailConfig.sh install"
RESULT=$?
...
if [[ $RESULT -ne 0 ]]; then
	if [[ -z "$1" ]]; then
		echo "Failed to configure Mail.  See messages above"
	fi
	exit 13
fi
